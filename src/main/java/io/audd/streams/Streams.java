...
public final class Streams {
    ...
    public void setCallbackUrl(String url) {
        setCallbackUrl(url, null, null);
    }

    public void setCallbackUrl(String url, List<String> returnMetadata) {
        setCallbackUrl(url, returnMetadata, null);
    }
     ...
     *   Typed params ({@code url}) win on collision.
     */
    public void setCallbackUrl(String url, List<String> returnMetadata, Map<String, String> extraParameters) {
        String mergedUrl = CallbackHelpers.addReturnToUrl(url, returnMetadata);
        Map<String, String> data = new HashMap<>();
        if (extraParameters != null) data.putAll(extraParameters);
    ...
    public void add(AddStreamRequest req) {
        Map<String, String> data = new HashMap<>();
        if (req.extraParameters() != null) data.putAll(req.extraParameters());
    ...
    /**
     * Long-poll for events on a stream by {@code radio_id}. Convenience
     * overload that derives the 9-char longpoll category from the
     * configured api_token + {@code radio_id} via
     * {@link #deriveLongpollCategory(int)}, then delegates to
     * {@link #longpoll(String)}.
     *
     * <p>Use the {@link #longpoll(String)} overload directly when the
     * caller has only a pre-derived category string (the tokenless
     * "share-with-browser/mobile/embedded" case).</p>
     */
    public LongpollPoll longpoll(int radioId) {
        return longpoll(deriveLongpollCategory(radioId), LongpollOptions.defaults());
    }

    public LongpollPoll longpoll(int radioId, LongpollOptions opts) {
