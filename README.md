...
Official Java SDK for [AudD](https://audd.io) — music recognition from a short audio clip, a long audio file, or a live stream.
...
```kotlin
implementation("io.audd:audd:1.4.5")
```
...
callback URL — parse the body with the static helper:
...
JsonNode body = readRequestBody(httpRequest);
StreamCallbackPayload payload = Streams.parseCallback(body);
if (payload.isResult()) {
    payload.result().results().forEach(s ->
        System.out.println(s.artist() + " — " + s.title()));
}
```

…or you long-poll for events. Each call blocks up to 50 seconds:

```java
import com.fasterxml.jackson.databind.JsonNode;

String category = audd.streams().deriveLongpollCategory(1234);
var iter = audd.streams().longpoll(category);
while (iter.hasNext()) {
...
Longpoll is the right primitive for browser widgets and embedded UIs:
the category string is the only credential the endpoint needs, so your
server can derive a category and ship just that to an untrusted client.
