...
[![CI](https://github.com/AudDMusic/audd-java/actions/workflows/ci.yml/badge.svg)](https://github.com/AudDMusic/audd-java/actions/workflows/ci.yml)
[![Contract](https://github.com/AudDMusic/audd-java/actions/workflows/contract.yml/badge.svg)](https://github.com/AudDMusic/audd-java/actions/workflows/contract.yml)
[![Maven Central](https://img.shields.io/maven-central/v/io.audd/audd.svg)](https://central.sonatype.com/artifact/io.audd/audd)
...
additional fields the SDK doesn't surface as typed properties:
...
`extraParameters` map via `RecognizeOptions` / `EnterpriseOptions`:
...
RecognitionResult r = audd.recognize(
    ...
    RecognizeOptions.builder()
        ...
        .extraParameters(Map.of("my_custom_flag", "1"))
        .build()
);
```
...
public class AudDCallbackController {
    ...
    public String handle(@RequestBody byte[] body) {
        CallbackEvent ev = Streams.parseCallback(body);
        ev.match().ifPresent(m -> { /* persist match */ });
        ev.notification().ifPresent(n -> { /* log notification */ });
        return "ok";
    }
}
```
...
Javalin.create().post("/audd/callback", ctx -> {
    var ev = Streams.parseCallback(ctx.bodyAsBytes());
    // handle ev as above
    ctx.result("ok");
}).start(8080);
```
...
```java
@WebServlet("/audd/callback")
public class AudDCallbackServlet extends HttpServlet {
    @Override
    protected void doPost(HttpServletRequest req, HttpServletResponse resp) throws IOException {
        try (var in = req.getInputStream()) {
            var ev = io.audd.streams.Streams.parseCallback(in);
            // handle ev
        }
        resp.setStatus(200);
    }
...
> longer audio files). The custom-catalog endpoint manipulates
